...
def plot_phase_plane(v,d):
    fig,ax=plt.subplots(figsize=(5,4));ax.plot(v,d,lw=1);ax.set_xlabel("Membrane potential (mV)");ax.set_ylabel("dV/dt");ax.grid(alpha=.3);return fig

st.set_page_config(page_title="AxoGraph Patch-Clamp Analyzer",layout="wide")
st.title("AxoGraph (.axgd) Current Clamp Analyzer")
st.caption("Passive properties come from the dedicated 1-channel test pulse; AP/AHP intrinsic properties use only APs generated at rheobase.")
with st.sidebar:
    ...
    pulse_mv=st.number_input("Voltage command amplitude (mV)",min_value=.1,max_value=100.,value=10.,step=.5,
        help="The supplied '0001 Test Pulse 1-Ch' protocol uses a voltage-command pulse. Change this only if your protocol command amplitude differs.")
...
if test_file is None or rheo_file is None:st.info("Upload both recordings to create a combined analysis/export.")
    ...
    try:
        ...
        tp=compute_test_pulse_properties(test_rec,pulse_amplitude_mv=pulse_mv);props=rheo_rec.properties
        ...
        with tab_tp:
            ...
            a,b,c,d=st.columns(4);a.metric("Average Rs",f"{_fmt(tp.series_resistance)} MOhm");b.metric("Average Rm",f"{_fmt(tp.membrane_resistance)} MOhm")
            c.metric("Average Cm",f"{_fmt(tp.membrane_capacitance)} pF");d.metric("Valid test-pulse sweeps",tp.n_valid_sweeps)
            if tp.sweep_values:st.dataframe(pd.DataFrame(tp.sweep_values),hide_index=True,use_container_width=True)
            else:st.error("No valid test-pulse transients were detected. Confirm this is a '0001 Test Pulse 1-Ch' recording and that the command amplitude above matches the protocol.")
        with tab_raw:
        ...
        with tab_summary:
            ...
            st.subheader("Combined Test Pulse + Rheobase Output");st.caption("SPIKE and AHP columns are averages of all APs generated in the rheobase sweep (one AP if rheobase produces one).")
        ...
        with tab_ap:
            if props.rheobase is None:st.info("No rheobase AP detected.")
                ...
                rb=next((s for s in rheo_rec.sweeps if np.isclose(s.step_amplitude,props.rheobase,atol=.5)),None)
                analysis=next((a for a in props.sweep_analyses if rb is not None and a.sweep_index==rb.index),None)
                if rb is not None and analysis is not None:
                    st.subheader(f"Rheobase sweep: {props.rheobase:.1f} pA | {analysis.n_spikes} AP(s)")
                    rows=[]
                    for i,sp in enumerate(analysis.spikes,1):rows.append({"AP":i,"Peak (mV)":sp.peak_voltage,"Threshold (mV)":sp.threshold_voltage,"Amplitude (mV)":sp.amplitude,"Half-width (ms)":sp.half_width,"Max rise (mV/ms)":sp.max_rise_slope,"Max fall (mV/ms)":sp.max_fall_slope,"AHP trough (mV)":sp.ahp_voltage})
                    if rows:
