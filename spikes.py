...
def _spike_features_at_peak(v,dv,fs,global_idx,next_global_idx,prev_peak_time,rise_thresh_frac=.10):
    peak_time=global_idx/fs;peak_v=float(v[global_idx])
    # Spreadsheet note specifies threshold at 10% of the AP's maximum first derivative.
    back=max(int(.005*fs),5);lo=max(global_idx-back,0);rise=dv[lo:global_idx+1]
    max_rel=int(np.argmax(rise));max_rise=float(rise[max_rel]);target=rise_thresh_frac*max_rise
    thr_idx=lo
    # Walk backward from maximum dV/dt to the first sub-threshold derivative.
    for j in range(max_rel,0,-1):
        if rise[j-1]<=target<rise[j]:thr_idx=lo+j-1;break
    threshold_v=float(v[thr_idx]);threshold_t=thr_idx/fs;amplitude=peak_v-threshold_v
    half_v=threshold_v+amplitude/2.
    li=global_idx
    ...
    # interpolate crossings for sampling-rate-independent width
    left_t=li/fs
    if li+1<len(v):left_t=_interp_cross(li/fs,v[li],(li+1)/fs,v[li+1],half_v)
    right_t=ri/fs
    if ri>0:right_t=_interp_cross((ri-1)/fs,v[ri-1],ri/fs,v[ri],half_v)
    half_width=(right_t-left_t)*1000. if right_t>left_t else np.nan
    max_fall=float(np.min(dv[global_idx:hi+1])) if hi>global_idx else np.nan
    # fAHP belongs to this AP: search only 0.25-20 ms after peak and never beyond
    # the next AP. This prevents a later slow drift from becoming the AHP trough.
    a0=min(global_idx+max(int(.00025*fs),1),len(v)-1)
    a1=min(global_idx+max(int(.020*fs),2),next_global_idx-1,len(v)-1)
    if a1>a0:
        rel=int(np.argmin(v[a0:a1+1]));ai=a0+rel;ahp_v=float(v[ai]);ahp_t=ai/fs
    ...
    return SpikeFeatures(peak_time=peak_time,peak_voltage=peak_v,threshold_voltage=threshold_v,threshold_time=threshold_t,
        amplitude=amplitude,half_width=half_width,max_rise_slope=max_rise,max_fall_slope=max_fall,
        ahp_voltage=ahp_v,ahp_time=ahp_t,isi_prev_ms=isi)
...
def detect_spikes(sweep:Sweep,min_peak_mv=-10.,min_prominence_mv=20.,refractory_ms=1.)->List[SpikeFeatures]:
    ...
    dv=np.gradient(v)*fs/1000. # mV/ms numerically equals V/s
    out=[];prev=None
...
def detect_spikes_template(sweep:Sweep,cfg:TemplateConfig=TemplateConfig(),template_ms=DEFAULT_TEMPLATE_MS,template_mv=DEFAULT_TEMPLATE_MV)->List[SpikeFeatures]:
    ...
    onset=sweep.step_onset_idx;ss=onset+int(cfg.latency_start_ms/1000*fs)
    se=min(onset+int(cfg.latency_end_ms/1000*fs),len(v)) if cfg.latency_end_ms is not None else (sweep.step_offset_idx or len(v))
...
def build_spike_event_features(v,fs,spike,step_onset_idx):
    # Conventional AP shape metrics: peak location, dV/dt threshold onset,
    # threshold-to-peak rise, half-amplitude width, and peak-to-threshold decay.
    step_t=step_onset_idx/fs*1000.;pi=int(round(spike.peak_time*fs));ti=int(round(spike.threshold_time*fs))
    decay_idx=pi
    limit=min(pi+int(.010*fs),len(v)-1)
    while decay_idx<limit and v[decay_idx]>spike.threshold_voltage:decay_idx+=1
    return SpikeEventFeatures(peak_voltage=spike.peak_voltage,location_ms=spike.peak_time*1000.-step_t,
        onset_ms=spike.threshold_time*1000.-step_t,rise_ms=(pi-ti)/fs*1000.,width_ms=spike.half_width,
        decay_ms=(decay_idx-pi)/fs*1000.)
...
def build_ahp_event_features(v,fs,spike,step_onset_idx,baseline_v=None):
    ...
    step_t=step_onset_idx/fs*1000.;pi=int(round(spike.peak_time*fs));tr=int(round(spike.ahp_time*fs))
    base=float(baseline_v) if baseline_v is not None else float(spike.threshold_voltage)
    # AHP onset is the post-spike downward crossing of the pre-step baseline.
    oi=pi
    for i in range(pi,tr+1):
        if v[i]<=base:oi=i;break
    amp=base-spike.ahp_voltage;half=base-amp/2.
    li=tr
    while li>oi and v[li]<half:li-=1
    hi=min(tr+int(.020*fs),len(v)-1);ri=tr
    while ri<hi and v[ri]<half:ri+=1
    return SpikeEventFeatures(peak_voltage=spike.ahp_voltage,location_ms=spike.ahp_time*1000.-step_t,
        onset_ms=oi/fs*1000.-step_t,rise_ms=(tr-oi)/fs*1000.,width_ms=(ri-li)/fs*1000.,decay_ms=(ri-tr)/fs*1000.)
